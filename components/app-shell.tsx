...
export function AppShell({ children }: { children: ReactNode }) {
    ...
    async function loadProfile() {
      try {
        ...
        const { data: profile } = await supabase
          ...
          .eq("id", user.id)
          .maybeSingle();
        if (active && profile?.full_name) setProfileName(profile.full_name);
      } catch {
        /* Keep the safe fallback when auth/profile data is unavailable. */
      }
    }
    void loadProfile();
    return () => {
  ...
  return (
    <div className="min-h-screen bg-[#f6f8f4]">
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-64 flex-col border-r border-[#e2e9e2] bg-[#fbfcfa] p-6 transition-transform lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}
      >
          ...
          {nav.map(({ href, label, icon: Icon }) => {
            ...
            return (
              <Link
                ...
                key={href}
                href={href}
                className={`flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition ${active ? "bg-[#e7f3d0] text-[#174b39]" : "text-[#7c8880] hover:bg-[#f0f4ed] hover:text-[#174b39]"}`}
              >
                <Icon size={18} />
                {label}
              </Link>
        ...
        <div className="mt-auto border-t border-[#e2e9e2] pt-5">
          <Link
            href="/settings"
            className="flex items-center gap-3 rounded-xl p-2 hover:bg-[#f0f4ed]"
          >
      ...
      <div className="lg:pl-64">
        <header className="flex h-16 items-center justify-between border-b border-[#e2e9e2] bg-[#fbfcfa] px-5 lg:hidden">
          ...
          <span className="h-8 w-8 rounded-full bg-[#d8e8cf] text-center text-sm font-bold leading-8 text-[#174b39]">
            {profileName.charAt(0).toUpperCase()}
          </span>
        </header>
